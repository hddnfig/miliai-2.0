...
작업 세션에서 제공된 ERD 이미지를 9개 영역으로 확대해 테이블·주요 컬럼·관계를 판독했다. 원본 이미지는 전달 저장소에 포함하지 않는다. ERD는 PBL, VOD, 회원·조직·권한, AI 대화, 동료평가, 진단·추천, 커뮤니티, 수료·크레딧, 알림·로그를 폭넓게 포함한다. MILI AI의 학습 핵심 흐름은 상당 부분 현재 DB로 시작할 수 있다.
